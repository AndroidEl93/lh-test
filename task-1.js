...
function sostavChisla(massivChisel, chislo) {
	//Сортировка вставками, отсеивая элементы > chislo
	let sort = [];
	for (let i = 0; i < massivChisel.length; i++) {
		let curValue = massivChisel[i];
		if (curValue <= chislo) {
			sort.push(curValue);
			let j = sort.length - 1;
			while (j > 0 && sort[j - 1] > curValue) {
				sort[j] = sort[j - 1];
				j--;
			}
			sort[j] = curValue;
	...
	/** Рекурсивный поиск комбинаций, по отсортированному массиву arr.
	 * @param {Array<number>} arr - Массив, в котором будет производиться поиск комбинаций.
	 * Должен быть отсортирован по возрастанию.
	 * @param {number} val - Число, которому должна быть равна сумма элементов найденный комбинаций.
	 * @param {Array<Array<number>>} [result=[]] - (Для рекурсивного вызова) Массив найденных комбинаций.
	 * @param {number} [start=0] - (Для рекурсивного вызова) С какого элемента начинать обход исходного массива.
	 * @param {number} [sum=0] - (Для рекурсивного вызова) Сумма элементов проверяемой комбинации.
	 * @param {Array<number>} [line=[]] - (Для рекурсивного вызова) Проверяемая комбинация.
	 * @returns {Array<Array<number>>} - Массив найденных комбинаций.
	 */
	function search(arr, val, result = [], start = 0, sum = 0, line = []) {
		for (let i = start; i < arr.length; i++) {
			let curSum = sum + arr[i];
			if (curSum === val) {
				result.push(line.concat(arr[i]));
			} else {
				if (curSum < val) {
					search(arr, val, result, i + 1, curSum, line.concat(arr[i]));
				} else {
					break;
				}
			}
